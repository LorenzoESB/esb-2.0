...
interface FinanciamentoImobiliarioOfertasProps {
  ofertas: OfertaFinanciamentoImobiliario[];
}
...
export function FinanciamentoImobiliarioOfertas({
  ofertas,
}: FinanciamentoImobiliarioOfertasProps) {
  const [sortField, setSortField] = useState<SortField>('parcelaInicial');
  ...
  // Melhor oferta é a primeira quando ordenado por parcela inicial crescente
  const melhorOferta = [...ofertas].sort(
    (a, b) => a.parcelaInicial - b.parcelaInicial
  )[0];

  const isMelhorOferta = (oferta: OfertaFinanciamentoImobiliario): boolean => {
    return (
      oferta.nomeBanco === melhorOferta.nomeBanco &&
      oferta.modalidade === melhorOferta.modalidade &&
      oferta.parcelaInicial === melhorOferta.parcelaInicial
    );
  };
      ...
      <CardContent>
            ...
            <TableHeader>
              <TableRow>
                ...
                <TableHead
                  ...
                  onClick={() => handleSort('parcelaInicial')}
                >
                ...
                <TableHead
                  className="cursor-pointer hover:bg-gray-50 min-w-[130px]"
                  onClick={() => handleSort('parcelaFinal')}
                >
                  <div className="flex items-center gap-1">
                    Última Parcela
                    <ArrowUpDown className="h-4 w-4" />
        ...
        <div className="mt-4 p-4 bg-gray-50 rounded-lg">
          ...
          <ul className="text-xs text-gray-600 space-y-1">
            <li>
              • <strong>1ª Parcela:</strong> Parcela mais alta no início do
              financiamento (Sistema SAC)
            </li>
            <li>
              • <strong>Última Parcela:</strong> Parcela mais baixa ao final do
              financiamento
            </li>
            <li>
              • <strong>Comprometimento:</strong> Percentual da renda mensal
              comprometido com a primeira parcela
            </li>
            <li>
              • <strong>Taxa:</strong> Taxas pós-fixadas, indexadas à TR (Taxa
              Referencial)
            </li>
          </ul>
        </div>
