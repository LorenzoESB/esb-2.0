...
describe('AmortizacaoService', () => {
  ...
  it('calculates a simplified amortization result', async () => {
    const input: AmortizacaoInputDto = {
      ...
      parcelaAtual: 28,
      amortizacoesExtraordinarias: [{ valor: 22000, mesOcorrencia: 1, tipo: TipoAmortizacaoExtraordinaria.DIMINUIR_PARCELA }],
    } as any;
  ...
  it('compares two simplified scenarios', async () => {
    ...
    expect(typeof result.analiseComparativa.diferencaPrestacao).toBe('number');
  });
});
